...
while True:
    ...
    if(rand()>min(tot_dep_mass/min_dep_mass, 1.0)):
        ...
        if(impactor.dx**2+impactor.dy**2 > 0):
            x_imp = xu/sin(ai) + xi[next_p]
            impactor.goto((x_imp ,0))
            impactor.color(sqrt(impactor.dx**2+impactor.dy**2)/vi_mag, 0, 0)
            x_exit = x_imp + exit_distance*cos(ar)
            y_exit = exit_distance*sin(ar)
            impactor.goto((x_exit,y_exit))
